/** Applies risk rules to normalized positions and produces a scan-level portfolio assessment. */
...
/** Classifies all positions in a scan and derives portfolio-level summary fields. */
...
/** Applies the configured rule set to a single normalized position. */
export function classifyPosition(
  ...
  // Missing stops are treated as an immediate warning even if the trade is otherwise healthy.
...
/** Collapses detailed risk reasons into one position-level risk label. */
...
/** Elevates the portfolio risk level to the highest severity seen across positions. */
...
/** Aggregates balance and PnL fields for the report and audit layers. */
...
/** Removes duplicate strings while keeping the first-seen order stable. */
