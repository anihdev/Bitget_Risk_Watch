...
/** Prints the structured scan result in a demo-friendly terminal layout. */
export function renderScanReport(scanRecord: ScanRecord): void {
  ...
  console.log(`\n${BOLD}Position Overview${RESET}`);
  for (const position of scanRecord.positions) {
    renderPositionOverview(position);
  }

  // Only flagged positions get the expanded explanation block to keep the terminal output compact.
  ...
  if (enrichments.length > 0) {
    console.log(`\n${BOLD}Optional Skill Hub Enrichments${RESET}`);
    for (const skill of enrichments) {
...
/** Prints a compact summary line for every open position, including safe ones. */
function renderPositionOverview(position: ClassifiedPosition): void {
  console.log(
    `- ${COLORS[position.riskLevel]}[${formatRiskLabel(position.riskLevel)}]${RESET} ${BOLD}${position.symbol}${RESET} | ${position.side} ${position.size} | leverage ${position.leverage.toFixed(1)}x | PnL ${formatUsd(position.unrealizedPnl)} (${position.unrealizedPnlPct.toFixed(1)}%) | margin ${position.marginRatio.toFixed(1)}%`,
  );
}

/** Prints the detailed rationale and actions for one flagged position. */
...
/** Applies a colored status label to Bitget skill call output. */
...
/** Formats USD values consistently for terminal output. */
...
/** Formats optional percentage values while preserving missing data as n/a. */
...
/** Explains the scan completeness state in plain language. */
...
/** Describes whether the Bitget CLI is available for live reads. */
...
/** Describes whether credentials are present, without assuming they are valid. */
